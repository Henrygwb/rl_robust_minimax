...
class SharedMemoryServer:
    ...
    def __init__(self, scene, game_server_guid, want_test_window,
                 steps=1, episode_start=1, file_name=None):
        self.scene = scene
        self.plist = []
        ...
        if file_name != None:
            ...
            self.fid = open(self.file_name, 'a+')
            self.fid.write("%d %d %d %f %f %f\n" % (0, 0, 0, 0.0, 0.0, 0.0))
            self.fid.close()
    ...
    def serve_forever(self):
        ...
        while still_open:
            ...
            while still_open:
                ...
                # print('score_board is ', self.scene.score_board)
                for p in self.plist:
                    p.read_and_apply_action()

                self.scene.global_step()
                frame += 1
                if episode >= self.episode_start:
                    total_steps += 1

                    left_score_list.append(self.scene.score_left - left_score)
                    left_score = self.scene.score_left

                    right_score_list.append(self.scene.score_right - right_score)
                    right_score = self.scene.score_right

                if episode >=self.episode_start and total_steps % self.steps == 0:

                    assert(len(left_score_list) == self.steps)
                    assert(len(right_score_list) == self.steps)
                    #
                    # print('-------------')
                    # print(left_score_list)
                    # print(right_score_list)
                    # print('-------------')

                    keys = ['oppo_double_hit', 'oppo_miss_catch', 'oppo_slow_ball']
                    win_0_num = 0
                    win_1_num = 0
                    for key in keys:
                        win_0_num += self.scene.score_board['left'][key]
                        win_1_num += self.scene.score_board['right'][key]
                    tie_num = abs(self.scene.score_board['left']['not_finish'])
                    self.fid = open(self.file_name, 'a+')
                    total_score = np.sum(np.array(left_score_list)) + np.sum(np.array(right_score_list))
                    self.fid.write("%d %d %d %f %f %f\n" % (win_0_num, win_1_num, tie_num, total_score, np.sum(np.array(left_score_list)), np.sum(np.array(right_score_list))))
                    self.fid.close()
