...
def symmtric_best_opponent(trainer, nupdates, select_workers, select_num_episodes, \
    ...
    save_filter = trainer.workers.local_worker().get_filters()[save_idx]
    ...
    for update in range(1, nupdates):
        ...
        # Load the obs_norm
        norm_path = os.path.join(out_dir, 'checkpoints', 'model', '%.5i' % update, 'obs_rms')
        tmp_filter = pickle.load(open(norm_path, 'rb'))
        select_workers.foreach_worker(lambda ev: ev.filters[load_idx].sync(tmp_filter))
...
def asymmtric_best_opponent(trainer, nupdates, start, select_workers, select_num_episodes, \
    ...
    for update in range(start, nupdates - 1, 2):
        ...
        select_workers.foreach_worker(lambda ev: ev.get_policy(load_idx).set_weights(tmp_model))
        # Load the obs_norm
        norm_path = os.path.join(out_dir, 'checkpoints', load_idx, '%.5i' % update, 'obs_rms')
        tmp_filter = pickle.load(open(norm_path, 'rb'))
        select_workers.foreach_worker(lambda ev: ev.filters[load_idx].sync(tmp_filter))
...
def assymmtric_learning(trainer, num_workers, nupdates, opp_method, select_num_episodes, out_dir):
